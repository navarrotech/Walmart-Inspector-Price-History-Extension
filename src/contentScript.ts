...
//   {
//       if (areaName === 'sync' && changes.featureEnabled)
//       {
//           const newValue = changes.featureEnabled.newValue as boolean
//           updateFeatureState(newValue)
//       }
//   })

(async function WalmartInspector(document: Document) {
  // ////////////////////// //
  //    Constants & Types   //
  // ////////////////////// //

  const NAME = 'walmartInspectorMainCS' as const
  const FAILED_PRICE_NUMBER = -1 as const
  ...
  function parseSafe<Type = Record<string, any>>(text: string): Type | undefined {
  ...
  function waitForElementToExist<Element = HTMLElement>(
    cssSelector: string,
    timeout?: number
  ...
  (async function walmartInspectorPriceReporterCS() {
    ...
    */
    ...
    /*
    * Notes:
    * We NEVER scan checkout pages
    * We NEVER scan account pages
    * We NEVER scan auth pages like login or signup
    ...
    */
    ...
    type PageFunction = (match: RegExpMatchArray) => Promise<UnmountFunction>
    type ItemReport = {
      itemName?: string
      itemId: string
      price: number
    }
    type FullItemReport = ItemReport & {
      storeId: string
    }
  ...
  (async function walmartManageThemes() {
    ...
    function loadTheme(theme: Types.Theme) {
    ...
    // Load initial theme
    const currentTheme = await chrome.storage.sync.get('theme') as Types.Storage
    loadTheme(currentTheme.theme)

    // Listen to changes
    chrome.storage.onChanged.addListener((changes, areaName) => {
      if (areaName === 'sync' && changes.theme) {
        const newValue = changes.theme.newValue as Types.Theme
        loadTheme(newValue)
      }
    })
  })()
